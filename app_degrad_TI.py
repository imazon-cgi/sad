...
def update_graphs(start_date, end_date, map_click_data, bar_click_data, total_bar_click_data, selected_state, selected_ti, reset_clicks, refresh_clicks, selected_states, selected_year, selected_ti_state):
    ...
    df_filtered = df_degrad[
        ((df_degrad['ANO'] == start_date.year) & (df_degrad['MES'] >= start_date.month)) |
        ((df_degrad['ANO'] > start_date.year) & (df_degrad['ANO'] < end_date.year)) |
        ((df_degrad['ANO'] == end_date.year) & (df_degrad['MES'] <= end_date.month))
    ]
    ...
    map_fig.update_layout(
        title={
            'text': f"Mapa de Degradação Ambiental (km²) - <br> {ti_title} ({start_date.strftime('%Y-%m')} a {end_date.strftime('%Y-%m')})",
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
            'font': {'size': 14}
    ...
    df_degrad_accum_total = df_filtered.groupby(['ANO', 'MES'])['AREAKM2'].sum().reset_index()
    ...
    while current_period_start < end_date:
        ...
        label = f"Degradação {current_period_start.year}-{current_period_end.year}"
    ...
    # Criando o gráfico de linha para visualização da degradação
    line_fig_period = px.line(df_combined, x='Meses', y='AREAKM2', color='Período', markers=True,
                              ...
                              title='Evolução da Degradação na Amazônia por Período')

    # Atualizando o título do gráfico de acordo com os filtros selecionados
    period_title = 'Evolução da Degradação na Amazônia por Período <br>'
    if selected_state:
...
def update_ti_options(selected_state):
    if selected_state:
        ...
        filtered_df = df_degrad[df_degrad['ESTADO'].isin(selected_state)]
        ...
        ti_options = [{'label': ti, 'value': ti} for ti in df_degrad['TERRA_INDI'].unique()]
        return ti_options
