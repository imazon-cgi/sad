...
def register_sad_desmatamento_estados(server):
    ...
    app = dash.Dash(
        __name__,
        server=server,
        url_base_pathname="/sad/desmatamento_estados/",
    ...
    df_degrad = pd.read_parquet(
        "https://github.com/imazon-cgi/sad/raw/refs/heads/main/datasets/csv/alertas_sad_desmatamento_08_2008_04_2024_municipios.parquet"
    )

    # pré-agregado para alguns gráficos
    df_acumulado_ano = (
    ...
    def update_bar_total(_):
        df_total = df_degrad.groupby("ANO")["AREAKM2"].sum().reset_index()
