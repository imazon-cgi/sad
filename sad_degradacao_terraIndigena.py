...
def update_graphs(selected_year, map_click_data, bar_click_data, total_bar_click_data, selected_state, reset_clicks, selected_states):
    ...
    bar_yearly_fig.update_layout(
        ...
        font=dict(size=10),
        title={
        'text': f'Taxas de Degradação Ambiental acumuladas - Terras Indígenas ({selected_year})' if not selected_state else f'Taxas de Degradação Ambiental acumuladas - Terras Indígenas ({", ".join(selected_state)}) ({selected_year})',
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top'
        }
    ...
    if selected_state:
        ...
        line_title = f'Taxas de Degradação Ambiental Terras Indígenas por Estado ({", ".join(selected_state)})'
    else:
    ...
    if selected_state and selected_states:
        title_text = f'Taxas de Degradação Ambiental acumuladas - Terras Indígenas ({", ".join(selected_states)}) ({", ".join(selected_state)})'
    elif selected_state:
        title_text = f'Taxas de Degradação Ambiental acumuladas - Terras Indígenas ({", ".join(selected_state)})'
    elif selected_states:
        title_text = f'Taxas de Degradação Ambiental acumuladas - Terras Indígenas ({", ".join(selected_states)})'
    else:
